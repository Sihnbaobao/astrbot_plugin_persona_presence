...
class SmartConcurrentManager:
    ...
    async def attach_payload(
        ...
        """Attach a payload only while its arrival registration is alive.

        Args:
            chat_id: Scoped conversation key.
            processing_id: Processing identifier of the registered message.
            content: Prepared message text.
            sender_name: Sender display name.
            sender_id: Sender identifier.
            cached_data: Prepared cache payload.
            is_forced: Whether this message creates a batch boundary.

        Returns:
            True when the payload was attached; otherwise False for a late attach.
        """
        try:
            lock = cls._get_lock()
            async with lock:
                cls._cleanup_expired_locked(chat_id)
                existing = cls._pending.get(chat_id, {}).get(processing_id)
                if existing is None:
                    return False
                cls._pending[chat_id][processing_id] = {
                    **existing,
    ...
    @classmethod
    async def clear_chat(cls, chat_id: str) -> None:
        """Clear pending and consumed state for one scoped conversation."""
        async with cls._get_lock():
            cls._pending.pop(chat_id, None)
            stale = [
                processing_id
                for processing_id, info in cls._consumed.items()
                if info.get("chat_id") == chat_id
            ]
            for processing_id in stale:
    ...
    async def claim_batch(
        ...
        try:
            ...
            async with lock:
                ...
                # A forced group anchor is an explicit boundary; private callers
                # pass max_batch_size to allow an intentional forced burst merge.
                if not (current_is_forced and max_batch_size is None):
                    for entry in ordered_entries[1:]:
                        ...
                        # Preserve forced messages as the next batch boundary.
                        if entry.get("is_forced", False) and max_batch_size is None:
                            break

                        # Arrival order is a barrier: do not skip an earlier
                        # message that has not finished payload preparation.
