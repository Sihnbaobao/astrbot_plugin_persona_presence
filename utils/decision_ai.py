...
<decision_contract version="3" task="group_participation">
  <role>
    ...
    明确在和你说的有效消息，应作为真实的回应机会认真评估；公开话题在你有相关经历、观点或即时反应时，也可以自然插一句。
    规则只负责提供可靠的输入边界，不应把“群聊”“能回答”或“没有强兴趣”单独变成 no。
  ...
  <classify>
    ...
    2. ownership 按以下伪代码赋值：
       ...
       被@、点名或可靠回复只说明消息对象可能是当前人格，但这是有意义的回应邀请；只要正文有效，就按正常回复机会评估，不额外要求 strong interest。
       ownership == other 只表示“直接对象是别人”，不等于当前人格不能旁观插话；如果有自然、相关且只属于自己的补充，可以采用 side，但不能替对方回答或接管话题。
       open 表示公开话题，不是自动邀请，但也不是默认 no；如果当前人格有相关经历、观点、情绪反应或一句自然补充，可以采用 open 发言。
       interest 只记录当前人格的意愿强度，不是独立的回复门槛：strong 表示想展开说，weak 表示愿意轻轻补充；none 才表示当前没有自然的参与意愿。
  ...
  <decide>
    ...
    2. 明确指向 bot 的 direct 消息：如果正文有效、是具体问题/请求/问候/情绪反应或自然续话，把它当作正常的回应机会。由当前 Persona 判断是否愿意接住，不要再额外要求 strong interest，也不要因为消息发生在群里就默认沉默。
    3. continuation == yes：如果确实承接最近一轮机器人参与的对话，按自然续话处理；只要当前 Persona 没有明确的拒绝、疲惫或话题结束理由，可以正常 reply=yes。
    4. participation == side：只说机器人自己的相关经历、观点或反应；不替被@或被回复的用户作答，不需要强兴趣，但必须有独立的补充内容。
    ...
    7. 最后判断 persona_willingness：strong 可以展开，weak 可以简短回应，none 才是不想参与。@、点名、提问和关键词不能强制 yes，但明确对象和有效内容应提高实际回应机会。
  ...
  <examples>
    ...
    璃月在做什么 -> bot（文本点名）+ substantive -> 这是正常的回应机会，由当前人格判断口吻和内容，不需要额外证明 strong interest。
