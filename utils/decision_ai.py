...
<decision_contract version="3" task="group_participation">
  <role>
    你是当前注入的人格本人，正在真实群聊中决定这次是否开口。
    群聊不是逐条自动回复，也不是默认沉默；先确认消息对象和说话姿态，再由人格的性格、兴趣、心情、关系、表达习惯和当前氛围决定是否愿意接话。
    明确在和你说的消息，是一次需要认真评估的回应邀请，不是必须接住的命令；公开话题在你有相关经历、观点或即时反应时，也可以自然插一句。
    规则只负责提供可靠的输入边界，不应把“群聊”或“能回答”单独变成 no；但@、点名和有效正文也不能把没有接话意愿的消息变成 yes。
    未开启人格时，使用自然、克制但愿意正常社交的普通人判断。
  </role>
  ...
  <classify>
    ...
    2. ownership 按以下伪代码赋值：
       ...
       被@、点名或可靠回复只说明消息对象可能是当前人格，是提高注意力的回应邀请，不是回复承诺。正文有效只表示值得评估，不等于当前人格现在愿意接话；即使是 direct，weak 或 none 也可以 no。
       ownership == other 只表示“直接对象是别人”，不等于当前人格一定要旁观插话；只有在有明确、相关且只属于自己的补充，并且人格自然想加入时，才可以采用 side。
       open 表示公开话题，不是自动邀请，但也不是默认 no；如果当前人格有相关经历、观点、情绪反应或一句自然补充，可以采用 open 发言。
       interest 记录当前人格的意愿强度并参与最终选择：strong 通常表示想展开说，weak 可能只想轻轻补充也可能保持安静，none 表示当前没有自然的参与意愿。
    ...
    4. participation 表示机器人可以采取的说话姿态：
       ...
       - open：当前消息没有特定对象，是一个可能的公共发言入口；如果当前人格有相关经历、观点或即时反应，可以自然开口，没有自然补充时再保持安静。
       - none：没有可靠或自然的发言入口。
  ...
  <decide>
    ...
    2. 明确指向 bot 的 direct 消息：如果正文有效、是具体问题/请求/问候/情绪反应或自然续话，把它当作需要认真评估的回应邀请。有效正文是必要条件，不是充分条件；当前 Persona 仍要判断自己现在是否真的想接话。interest=weak 时，只有具体问题、明确请求、必要澄清或自然续话才有充分理由 reply=yes；泛泛召唤、简单反应、例行问候、重复催促或只是等待机器人回应通常 reply=no；interest=none 应 no。不要因为没有@就自动沉默，也不要因为有@就自动 yes。
    3. continuation == yes：如果确实承接最近一轮机器人参与的对话，按自然续话处理；仍由当前 Persona 判断是否愿意继续，不能仅因发送者相同或消息有内容就自动 reply=yes。
    4. participation == side：只说机器人自己的相关经历、观点或反应；不替被@或被回复的用户作答。必须同时有清楚的独立补充和自然的参与意愿，否则 reply=no。
    5. ownership == open：如果当前人格对公开话题有具体的共同兴趣、个人经验、观点或即时情绪，可以自然说一两句。open 不是自动邀请，但也不是默认 no；不要因为“这只是群聊”或“别人可能也能回答”而自动放弃。
    6. information 是描述，不是机械门槛：noise 通常 no；reaction 可以安静，也可以在当前人格确实想接话时简短回应；substantive 应结合对象和人格意愿认真评估。
    7. 最后判断 persona_willingness：strong 通常可以展开，weak 可以简短回应也可以保持安静，none 应 no。@、点名、提问和关键词只能提高注意力，不能强制 yes；明确对象和有效内容也不能替代当前 Persona 的接话意愿。

    最终结果：reply = persona_willingness，但前面的消息对象和说话姿态硬边界优先。目标是像真实群成员一样有选择地参与，而不是把“谨慎”执行成几乎永远不说话。
  </decide>

  <examples>
    好无聊啊 -> open + substantive -> 如果当前人格有自然的安慰、吐槽或分享冲动，可以接一句；没有想说的内容才 no。
    璃月在做什么 -> bot（文本点名）+ substantive -> 是需要认真评估的回应邀请；如果当前人格有接话意愿可以回答，没有自然意愿时仍可 no。
    是吗 / 奇怪 / 歌 -> reaction -> 可以保持安静，也可以在当前人格有即时反应时简短接话。
    那是什么歌 -> 如果近期机器人回复唯一提到一首歌，且中间插话没有接管话题，可以 continuation=yes；如果只是旧历史提过，或群聊已经明显转向，应按当前消息重新判断。
    地震了 / Miku好可爱 -> open + substantive -> 如果当前人格有感受、观点或经历，可以自然说一两句。
    九月有什么好看的番吗 -> open + substantive -> 如果当前人格有推荐、偏好或个人经验，可以自然回答一两句；没有想分享时才 no。
    有木有小的蓝牙耳机推荐的 -> open + substantive -> 如果当前人格确实有耳机使用体验，可以直接分享一两句；是否展开仍由人格决定。
    @小明你几点到 -> other + substantive，但只是等待小明回答 -> participation=none -> no。
    @小明这个游戏我也玩过 -> other + substantive -> 只有人格确实想补充自己的体验时才 participation=side，否则 no。
    回复小明：哈哈 -> other + reaction -> 通常没有独立补充，应 participation=none；只有当前人格有明确且自然的独立反应时才考虑 side。
...
<decision_contract version="4" task="private_participation">
  ...
  <input>
    ...
    <current_time>
      如果输入提供了“当前现实时间”，把它作为背景因素，与当前 Persona 的性格、心情、关系和边界一起判断当下是否愿意回应；时间只能参与权衡，不能替代人格判断。
    </current_time>
  </input>
  ...
  <decide>
    1. 普通开放私聊
       问候、提问、请求、解释、分享经历和正在进行的自然续话都应认真评估，但最终由当前 Persona 的性格、心情、关系、边界和消息具体内容决定是否愿意回应；不要把“有内容”机械等同于 yes。
       时间只是背景因素：它可以影响打扰成本和当下状态，但不能单独把消息判为 yes 或 no。
       安静、冷淡或话少的人格可以用更短、更克制的正式回复，也可以选择不参与；不要把所有私聊机械判为 yes 或 no。
        ...
        try:
            ...
            if not is_private:
                enhanced_context += (
                    ...
                    "不能直接断言消息没有指向机器人。普通历史、AstrBot 官方群聊上下文、【📦近期未回复】缓存和长期记忆只能作为背景，"
            ...
            if include_current_time:
                try:
                    ...
                    if is_private:
                        time_guidance = (
                            "这是当前真实时间，请把它作为背景而不是拒绝理由。"
                            "在一对一私聊中，不要仅因深夜或清晨把正常直接消息判为 no；"
                            "没有 active sleep/avoidance boundary 时，正常问候、提问、分享和自然续话仍按可回应内容处理。\n"
                        )
                    else:
                        time_guidance = (
                            "这是当前真实时间，请把深夜、清晨、工作时间和打扰成本纳入人格的当下判断。"
                            "时间是背景而不是绝对禁言规则：群聊在深夜可以更克制，但不能机械地拒绝明确重要、紧急或值得回应的消息。\n"
                        )
                    current_time_context = (
                        "\n\n[系统信息-当前现实时间] "
