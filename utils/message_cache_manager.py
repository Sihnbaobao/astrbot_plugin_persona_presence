...
class MessageCacheManager:
    ...
    def add_to_cache(
        ...
        #   image_urls          → 保留（lazy 模式需要在后续被接管时重新合并）
        ...
        image_urls = [url for url in (message_data.get("image_urls") or []) if url]
        message_data["image_urls"] = image_urls

        # Keep an image-only entry when a deferred URL is available; discard
        # placeholders that carry neither usable text nor a retrievable image.
        if not message_data.get("content", "") and not image_urls:
