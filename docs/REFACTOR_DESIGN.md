...
- weak：Persona 没有强烈冲动，可能只想轻轻补充一句，也可能选择安静；strong、weak 和 none 都是意愿描述，不再由本地代码机械决定能不能发言，@ 也不能把 weak 自动变成 yes。
...
- ambient：普通群消息可以进入统一参与判断；未明确指向机器人的明显纯媒体、低信息反应和只等待其他人的无正文消息可在模型前快速过滤。被 @、点名、关键词或可靠回复信号明确指向的纯媒体仍可继续图片处理和参与判断。明确指向机器人的有效消息进入认真评估，但仍由 Persona 读空气决定是否接住；开放话题有具体个人补充或自然反应时也可以参与。
...
观察缓存的用途是保留事件痕迹和避免消息完全消失，不是堆积“机器人欠下的回答”。active 缓存读取接口会排除 decision_state=observed：
...
群聊参与判断不读取这套插件缓存，而是读取 AstrBot 官方 GroupChatContext 当前事件之前的缓冲；正式回复继续由 AstrBot 的官方 LLM hook 注入同一缓冲。这样既保留群聊背景，也不让 observation-only 消息变成待回复任务。
